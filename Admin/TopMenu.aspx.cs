...
public partial class Admin_TopMenu : System.Web.UI.Page
{
    ...
    public void OnMenuChanged(object sender, EventArgs e)
    {
        using (DBDataContext db = new DBDataContext())
        {
            ...
            var p = (from P in db.Pages
                     join TM in db.TopMenuItem on P.id equals TM.PageId
                     where TM.id == mId
            ...
            using (DBDataContext db = new DBDataContext())
            {
                ...
                if (int.TryParse(ddlMenu.SelectedValue, out mid) && int.TryParse(ddlPage.SelectedValue, out pid))
                {
                    var p = (from P in db.Pages
                             where P.id == pid
                             select P).FirstOrDefault();
                    var m = (from M in db.TopMenuItem
                             where M.id == mid
                    ...
                    if (p != null && m != null)
                    {
                        m.PageId = p.id;
                        db.SubmitChanges();
                        lblMessage.Text = string.Format("Пункт меню '{0}' был успешно связан со страницей '{1}'", m.Title, p.Title);
                    }
                }
            }
        }
    ...
    private void InitLists()
    {
        using (DBDataContext db = new DBDataContext())
        {
            var menudata = from LM in db.TopMenuItem
                           orderby LM.Order
                           select LM;
    ...
    public void OnDelete(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(ddlMenu.SelectedValue, out i))
        {
            using (DBDataContext db = new DBDataContext())
            {
                ...
                if (m != null)
                {
                    db.TopMenuItem.DeleteOnSubmit(m);
                    db.SubmitChanges();
                    lblMessage.Text = string.Format("Пункт меню '{0}' удален.", m.Title);
                    InitLists();
                }
            }
        }
    ...
    public void OnUpdate(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(ddlMenu.SelectedValue, out i) && txtNewMenu.Text.Trim().Length > 0)
        {
            using (DBDataContext db = new DBDataContext())
            {
                ...
                if (m != null)
                {
                    string oldName = m.Title;
                    m.Title = txtNewMenu.Text.Trim();
                    db.SubmitChanges();
                    lblMessage.Text = string.Format("Пункт меню '{0}' переименован в '{1}'.", oldName, m.Title);
                    InitLists();
                }
            }
        }
    ...
    public void OnLeft(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(ddlMenu.SelectedValue, out i))
        {
            using (DBDataContext db = new DBDataContext())
            {
                ...
                if (m != null)
                {
                    var exm = (from EXM in db.TopMenuItem
